...
@mock_aws
def test_handler_png(mocker, get_file):
    test_bucket = "test-bucket"
    test_key = "test-image.png"
    test_file = get_file("./resources/hatsune_miku.png")

    run_test(test_bucket, test_key, test_file)


@mock_aws
def test_handler_jpg(mocker, get_file):
    test_bucket = "test-bucket"
...
def run_test(test_bucket, test_key, test_file):
    """
    Upload a file from resources/ so boto3.client can download it.
    """
    client = boto3.client("s3", region_name="us-east-1")
    client.create_bucket(Bucket=test_bucket)
    client.put_object(Bucket=test_bucket, Key=test_key, Body=test_file)
